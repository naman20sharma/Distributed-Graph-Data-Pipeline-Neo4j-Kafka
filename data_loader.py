...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        # TODO: Your code here
        # Connect to Neo4j session
        with self.driver.session() as session:
            # Create 'Location' nodes for each unique LocationID
            unique_locations = set(trips['PULocationID']).union(set(trips['DOLocationID']))
            for location_id in unique_locations:
                session.run("""
                    MERGE (location:Location {name: $location_id})
                """, location_id=int(location_id))

            # Create 'TRIP' relationships between 'Location' nodes
            for index, row in trips.iterrows():
                ...
                pickup_datetime=row['tpep_pickup_datetime'].to_pydatetime(),
                dropoff_datetime=row['tpep_dropoff_datetime'].to_pydatetime())
